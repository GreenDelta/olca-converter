...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ilcd="http://lca.jrc.it/ILCD" xmlns:process="http://lca.jrc.it/ILCD/Process"
  ...
  <xsl:template name="checkDependency">
    ...
    <xsl:if test="count($dependsOn)=0">
      <xsl:message><xsl:if test="$messagePrefix">
          <xsl:value-of select="$messagePrefix"/>
          <xsl:text>: </xsl:text>
        </xsl:if>If <xsl:value-of select="$elementDesc"/> is present, <xsl:value-of select="$dependsOnDesc"/> must be specified.</xsl:message>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="checkPresence">
    ...
    <xsl:if test="count($element)=0">
      <xsl:message><xsl:if test="$messagePrefix">
          <xsl:value-of select="$messagePrefix"/>
          <xsl:text>: </xsl:text>
        </xsl:if>Element <xsl:value-of select="$elementDesc"/> must be present.</xsl:message>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="checkValue">
    ...
    <xsl:if test="not($element=$value1) and not($element=$value2)">
      <xsl:message><xsl:if test="$messagePrefix">
          <xsl:value-of select="$messagePrefix"/>
          <xsl:text>: </xsl:text>
        </xsl:if>Value of <xsl:value-of select="$elementDesc"/> must be "<xsl:value-of select="$value1"/>"<xsl:if test="not($value2='')"> or "<xsl:value-of select="$value2"
          />"</xsl:if>.</xsl:message>
    </xsl:if>
  </xsl:template>
...
-->      
  ...
  <xsl:template name="equalsUUID">
    ...
    <xsl:value-of select="boolean(translate($uuid1, 'abcdef', 'ABCDEF') = translate($uuid2, 'abcdef', 'ABCDEF'))"/>
  </xsl:template>
  ...
  <xsl:template name="validationEvent">
    ...
    <xsl:message>Validation error: <xsl:value-of select="$message"/></xsl:message>
    <xsl:if test="$fancyMessages='true'">
      <xsl:call-template name="line"/>
    </xsl:if>
  </xsl:template>
