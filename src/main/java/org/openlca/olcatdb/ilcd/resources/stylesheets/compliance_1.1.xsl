...
<!-- ILCD Format Version 1.1 Tools Build 987 -->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:categories="http://lca.jrc.it/ILCD/Categories"
  ...
  <xsl:variable name="requiredComplianceSystem" select="'ILCD Compliance 1.1'"/>
  <xsl:variable name="requiredComplianceSystemDataSetUUID" select="'88d4f8d9-60f9-43d1-9ea3-329c10d7d727'"/>
  ...
  <xsl:variable name="categoriesFile" select="'ILCDClassification_Reference.xml'"/>
  <xsl:variable name="validCategories" select="document($categoriesFile)/categories:CategorySystem"/>
  ...
  <xsl:template name="checkReferenceToComplianceSystem">
    ...
    <xsl:variable name="referencePosition">
      <xsl:for-each select="/*/*[local-name()='modellingAndValidation']/*[local-name()='complianceDeclarations']/*[local-name()='compliance']">
        <xsl:variable name="matchRefObjectId">
          <xsl:call-template name="equalsUUID">
            <xsl:with-param name="uuid1" select="*[local-name()='referenceToComplianceSystem']/@refObjectId"/>
    ...
    <xsl:for-each
      ...
      <xsl:choose>
        ...
        <xsl:when test="@refObjectId and @version and not(@refObjectId='') and not(@version='')">
          ...
          <xsl:choose>
            <xsl:when test="$uriExists='true'">
              <xsl:variable name="complianceSystem"
                select="document($uri, /)/source:sourceDataSet/source:sourceInformation/source:dataSetInformation/common:shortName[@xml:lang='en']"/>
              <xsl:if test="not($complianceSystem=$requiredComplianceSystem)">
                <xsl:message><xsl:text>General compliance: referenceToComplianceSystem: expected '</xsl:text><xsl:value-of select="$requiredComplianceSystem"
                    /><xsl:text>',	found '</xsl:text><xsl:value-of select="$complianceSystem"/>'</xsl:message>
              </xsl:if>
            </xsl:when>
            <xsl:otherwise>
              <xsl:message>
  ...
  <xsl:template name="process-documentationCompliance">
    ...
    <xsl:call-template name="checkPresence">
      <xsl:with-param name="element" select="//process:classificationInformation/common:class[@level=0]"/>
      <xsl:with-param name="elementDesc" select="'category with level 0'"/>
      <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
    </xsl:call-template>

    <xsl:call-template name="checkPresence">
      <xsl:with-param name="element" select="//process:classificationInformation/common:class[@level=1]"/>
      <xsl:with-param name="elementDesc" select="'category with level 1'"/>
      <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
    </xsl:call-template>

    <xsl:call-template name="checkPresence">
      <xsl:with-param name="element" select="//process:locationOfOperationSupplyOrProduction/process:descriptionOfRestrictions"/>
      <xsl:with-param name="elementDesc" select="'locationOfOperationSupplyOrProduction/descriptionOfRestrictions'"/>
      <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
    </xsl:call-template>
    <xsl:call-template name="checkPresence">
      <xsl:with-param name="element" select="//process:technologyDescriptionAndIncludedProcesses"/>
      <xsl:with-param name="elementDesc" select="'technologyDescriptionAndIncludedProcesses'"/>
      <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
    </xsl:call-template>
    <xsl:call-template name="checkPresence">
    ...
    <xsl:call-template name="checkPresence">
      <xsl:with-param name="element" select="//process:referenceToTechnologyFlowDiagrammOrPicture"/>
    ...
    <xsl:call-template name="checkPresence">
      <xsl:with-param name="element" select="//process:deviationsFromdataCutOffAndCompletenessPrinciples"/>
    ...
    <xsl:call-template name="checkPresence">
      <xsl:with-param name="element" select="//process:deviationsFromDataSelectionAndCombinationPrinciples"/>
      <xsl:with-param name="elementDesc" select="'deviationsFromDataSelectionAndCombinationPrinciples'"/>
      <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
    </xsl:call-template>
    <xsl:call-template name="checkPresence">
      <xsl:with-param name="element" select="//process:dataTreatmentAndExtrapolationsPrinciples"/>
      <xsl:with-param name="elementDesc" select="'dataTreatmentAndExtrapolationsPrinciples'"/>
      <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
    </xsl:call-template>
    <xsl:call-template name="checkPresence">
      <xsl:with-param name="element" select="//process:deviationsFromDataTreatmentAndExtrapolationsPrinciples"/>
      <xsl:with-param name="elementDesc" select="'deviationsFromDataTreatmentAndExtrapolationsPrinciples'"/>
      <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
    </xsl:call-template>
    <xsl:call-template name="checkPresence">
      <xsl:with-param name="element" select="//process:dataSourcesTreatmentAndRepresentativeness/process:referenceToDataSource"/>
...
-->
    ...
    <xsl:if test="not(count(//process:mathematicalRelations/*)=0)">
      <xsl:call-template name="checkDependency">
        ...
        <xsl:with-param name="dependsOn" select="//process:mathematicalRelations/process:modelDescription"/>
      ...
      <xsl:for-each select="//process:mathematicalRelations/process:variableParameter">
        <xsl:call-template name="checkDependency">
          ...
          <xsl:with-param name="dependsOnDesc" select="'on each variableParameter element a meanValue attribute'"/>
          <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
        </xsl:call-template>
        <xsl:call-template name="checkDependency">
          <xsl:with-param name="elementDesc" select="'any field in mathematicalRelations'"/>
          <xsl:with-param name="dependsOn" select="@variableParameter"/>
          <xsl:with-param name="dependsOnDesc" select="'on each variableParameter element a variableParameter attribute'"/>
          <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
        </xsl:call-template>
        <xsl:call-template name="checkDependency">
          <xsl:with-param name="elementDesc" select="'any field in mathematicalRelations'"/>
          <xsl:with-param name="dependsOn" select="@comment"/>
          <xsl:with-param name="dependsOnDesc" select="'on each variableParameter element a comment attribute'"/>
          <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
        </xsl:call-template>
      </xsl:for-each>
    </xsl:if>

    <xsl:if test="//process:typeOfDataSet='Unit process, not pre-allocated' or //process:typeOfDataSet='Pre-allocated unit process'">
      <xsl:call-template name="checkDependency">
        <xsl:with-param name="elementDesc">process type 'Unit process, not pre-allocated' or 'Pre-allocated unit process'</xsl:with-param>
  ...
  <xsl:template name="process-reviewCompliance">
    ...
    <xsl:call-template name="checkPresence">
      <xsl:with-param name="element" select="//process:review[@type='Independent review panel']"/>
      <xsl:with-param name="elementDesc">review of type 'Independent review panel'</xsl:with-param>
      <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
    </xsl:call-template>

    <xsl:call-template name="checkPresence">
      <xsl:with-param name="element" select="//process:review[@type='Independent review panel']/common:scope"/>
      <xsl:with-param name="elementDesc">scope for review of type 'Independent review panel'</xsl:with-param>
      <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
    </xsl:call-template>

    <xsl:call-template name="checkPresence">
      <xsl:with-param name="element" select="//process:review[@type='Independent review panel']/common:scope[@name='Unit process(es)']"/>
      <xsl:with-param name="elementDesc">scope 'Unit process(es)' for review of type 'Independent review panel'</xsl:with-param>
      <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
    </xsl:call-template>
    ...
    <xsl:call-template name="checkPresence">
      ...
      <xsl:with-param name="elementDesc">method 'Compliance with ISO 14040 to 14044' for scope 'Unit process(es)' for review of type 'Independent review panel'</xsl:with-param>
      <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
    </xsl:call-template>

    <xsl:call-template name="checkPresence">
      <xsl:with-param name="element" select="//process:review[@type='Independent review panel']/common:scope[@name='Unit process(es)']/common:method[@name='Energy balance']"/>
      <xsl:with-param name="elementDesc">method 'Energy balance' for scope 'Unit process(es)' for review of type 'Independent review panel'</xsl:with-param>
      <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
    </xsl:call-template>

    <xsl:call-template name="checkPresence">
      <xsl:with-param name="element" select="//process:review[@type='Independent review panel']/common:scope[@name='Unit process(es)']/common:method[@name='Element balance']"/>
      <xsl:with-param name="elementDesc">method 'Element balance' for scope 'Unit process(es)' for review of type 'Independent review panel'</xsl:with-param>
      <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
    </xsl:call-template>
    ...
    <xsl:call-template name="checkPresence">
      ...
      <xsl:with-param name="elementDesc">method 'Cross-check with other dataset', 'Compliance with legal limits' or 'Cross-check with other source' for scope 'Unit process(es)' for
    ...
    <xsl:call-template name="checkPresence">
      ...
      <xsl:with-param name="elementDesc">method 'Compliance with ISO 14040 to 14044' for scope 'LCI method' for review of type 'Independent review panel'</xsl:with-param>
      <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
    </xsl:call-template>

    <xsl:call-template name="checkPresence">
      <xsl:with-param name="element" select="//process:review[@type='Independent review panel']/common:scope[@name='Documentation']/common:method[@name='Expert judgement']"/>
      <xsl:with-param name="elementDesc">method 'Expert judgement' for scope 'Documentation' for review of type 'Independent review panel'</xsl:with-param>
      <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
    </xsl:call-template>
    ...
    <xsl:for-each select="//process:review[@type='Independent review panel']">
      <xsl:call-template name="checkPresence">
        ...
        <xsl:with-param name="elementDesc">reviewDetailsOnTechnicalContent for review of type 'Independent review panel'</xsl:with-param>
        <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
      </xsl:call-template>
      <xsl:call-template name="checkPresence">
        <xsl:with-param name="element" select="process:reviewDetailsOnImpactCoverage"/>
        <xsl:with-param name="elementDesc">reviewDetailsOnImpactCoverage for review of type 'Independent review panel'</xsl:with-param>
        <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
      </xsl:call-template>
      <xsl:call-template name="checkPresence">
        <xsl:with-param name="element" select="process:reviewDetailsOnLCIAMethod"/>
        <xsl:with-param name="elementDesc">reviewDetailsOnLCIAMethod for review of type 'Independent review panel'</xsl:with-param>
        <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
      </xsl:call-template>
      <xsl:call-template name="checkPresence">
        <xsl:with-param name="element" select="common:referenceToNameOfReviewerAndInstitution"/>
        <xsl:with-param name="elementDesc">referenceToNameOfReviewerAndInstitution for review of type 'Independent review panel'</xsl:with-param>
        <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
      </xsl:call-template>
      <xsl:call-template name="checkPresence">
        <xsl:with-param name="element" select="common:referenceToNameOfReviewerAndInstitution/common:shortDescription"/>
        <xsl:with-param name="elementDesc">shortDescription for referenceToNameOfReviewerAndInstitution for review of type 'Independent review panel'</xsl:with-param>
        <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
      </xsl:call-template>
      <xsl:call-template name="checkPresence">
        <xsl:with-param name="element" select="common:referenceToCompleteReviewReport"/>
        <xsl:with-param name="elementDesc">referenceToCompleteReviewReport for review of type 'Independent review panel'</xsl:with-param>
        <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
      </xsl:call-template>
    </xsl:for-each>
  ...
  <xsl:template name="flow-documentationCompliance">
    ...
    <xsl:variable name="typeOfDataset" select="flow:modellingAndValidation/flow:LCIMethod/flow:typeOfDataSet/text()"/>
  ...
  <xsl:template name="flow-nomenclatureAndHierarchyCompliance">
    ...
    <xsl:call-template name="checkPresence">
      <xsl:with-param name="element" select="//flow:classificationInformation/common:class[@level=0]"/>
      <xsl:with-param name="elementDesc" select="'category with level 0'"/>
      <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
    </xsl:call-template>
  ...
  <xsl:template name="flowproperty-nomenclatureAndHierarchyCompliance">
    ...
    <xsl:call-template name="checkPresence">
      <xsl:with-param name="element" select="//flowproperty:classificationInformation/common:class[@level=0]"/>
      <xsl:with-param name="elementDesc" select="'category with level 0'"/>
      <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
    </xsl:call-template>
    ...
    <xsl:for-each select="//flowproperty:classificationInformation">
      <xsl:call-template name="check_hierarchy">
        <xsl:with-param name="tree" select="$validCategories/categories:flowPropertyCategories"/>
        <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
      </xsl:call-template>
    </xsl:for-each>

    <xsl:call-template name="checkValue">
      <xsl:with-param name="element" select="//flowproperty:nomenclatureAndHierarchyCompliance"/>
      <xsl:with-param name="elementDesc" select="'nomenclatureAndHierarchyCompliance'"/>
      <xsl:with-param name="value1" select="'Fully conform'"/>
      <xsl:with-param name="messagePrefix" select="$msgPrefix"/>
    </xsl:call-template>
