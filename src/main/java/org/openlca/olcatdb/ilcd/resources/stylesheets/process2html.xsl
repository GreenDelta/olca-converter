...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ilcd="http://lca.jrc.it/ILCD"
  ...
  <xsl:template match="process:mathematicalRelations">
    ...
    <xsl:if test="child::*">
      ...
      <xsl:variable name="variableElements"
        select="'variableParameter,formula,meanValue,minimumValue,maximumValue,uncertaintyDistributionType,relativeStandardDeviation95In,comment'"/>
  ...
  <xsl:template name="getFlowDatasetClassification">
    ...
    <xsl:if test="$doNotLoadExternalResources!='true'">
      ...
      <xsl:for-each
        select="$flowDataset/flow:flowInformation/flow:dataSetInformation/flow:classificationInformation/*[local-name()=$classificationElementName and namespace-uri()='http://lca.jrc.it/ILCD/Common' and (@name='ILCD' or not(@name))]/*">
