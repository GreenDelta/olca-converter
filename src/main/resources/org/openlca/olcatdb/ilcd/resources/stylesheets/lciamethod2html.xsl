...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:lciamethod="http://lca.jrc.it/ILCD/LCIAMethod"
  ...
  <xsl:template match="lciamethod:locationOfValidity">
  ...
  <xsl:template match="lciamethod:subLocationOfValidity">
    <xsl:call-template name="masterTemplate">
      <xsl:with-param name="omitIfEmpty" select="false()"/>
      <xsl:with-param name="data">
  ...
  <xsl:template match="lciamethod:flowsExchanges">
    <!-- holds the elements to display in the order they are supposed to appear in -->
    <xsl:variable name="flowExchangeElements"
      select="'referenceToFlowDataSet,location,exchangeDirection,meanValue,minimumValue,maximumValue,uncertaintyDistributionType,relativeStandardDeviation95In,dataDerivationTypeStatus'"/>
    <tr>
      ...
      <td colspan="2">
        <table width="100%">
          ...
          <xsl:call-template name="printFlowsExchangesHeaderRow">
            <xsl:with-param name="flowExchangeElements" select="$flowExchangeElements"/>
          </xsl:call-template>
          ...
          <xsl:for-each select="*">
            <tr>
              ...
              <xsl:call-template name="processFlowsExchangesData">
                <xsl:with-param name="list" select="$flowExchangeElements"/>
              </xsl:call-template>
            </tr>
  ...
  <xsl:template name="printFlowsExchangesHeaderRow">
    <xsl:param name="flowExchangeElements"/>
    <tr>
