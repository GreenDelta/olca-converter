...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:categories="http://lca.jrc.it/ILCD/Categories"
  ...
  <xsl:template match="*[local-name()='referenceToTechnologyPictogramme' or local-name()='referenceToTechnologyFlowDiagrammOrPicture' or local-name()='referenceToLCIAMethodFlowDiagrammOrPicture' or local-name()='referenceToLogo']"
    priority="20">
  ...
  <xsl:template name="getElementType">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$elementName='LCIAResult'">
  ...
  <xsl:template name="getComment">
    ...
    <xsl:variable name="result">
      <xsl:choose>
        <!-- resolve ambiguities -->
        <xsl:when test="$fieldName='name' or $fieldName='generalComment' or $fieldName='flowProperties' or $fieldName='referenceToDataSource'">
          <xsl:variable name="parentType">
            <xsl:call-template name="getElementType">
              <xsl:with-param name="elementName" select="$parent"/>
            </xsl:call-template>
  ...
  <xsl:template name="getSourceDatasetCategory">
    ...
    <xsl:if test="$doNotLoadExternalResources!='true'">
      <xsl:value-of
        select="document($sourceDatasetURI, /)/source:sourceDataSet/source:sourceInformation/source:dataSetInformation/source:classificationInformation/common:class[@level='0']"/>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="getFlowPropertyName">
    ...
    <xsl:choose>
      <xsl:when
        test="$flowPropertyDataset/flowproperty:flowPropertyDataSet/flowproperty:flowPropertiesInformation/flowproperty:dataSetInformation/common:name[@xml:lang=$defaultLanguage]">
        <xsl:value-of
  ...
  <xsl:template name="getUnitGroupName">
    <xsl:param name="unitGroupDataset"/>
    <xsl:value-of
      select="$unitGroupDataset/unitgroup:unitGroupDataSet/unitgroup:unitGroupInformation/unitgroup:dataSetInformation/common:name"
    />
  </xsl:template>
